...
parts = content.split('<Header onHome={goHome} />')
before_header = parts[0] + '<Header onHome={goHome} />\n      '

after_header = parts[1]
# find the last </div>\n  );
end_idx = after_header.rfind("</div>\n  );")

end_part = after_header[end_idx:]
...
full_content = before_header + new_render + end_part

with open(app_path, 'w') as f:
    f.write(full_content)

print(f"Animation patched successfully via string splitting.")
