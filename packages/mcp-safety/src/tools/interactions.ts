...
export function registerInteractionTools(server: McpServer): void {
  server.tool(
    ...
    "Get known drug interaction information for a drug. Accepts either an RxCUI (resolved to drug name via RxNav) or a drug name directly. Returns drug interaction text from FDA drug labels.",
    {
      drug: z
        .string()
        .describe(
          "Drug name (e.g. 'aspirin') or RxNorm Concept Unique Identifier (RxCUI)",
        ),
    },
    async ({ drug }) => {
      try {
        ...
        if (/^\d+$/.test(drug)) {
          ...
          const props = data?.propConceptGroup as
            | { propConcept?: Array<{ propName?: string; propValue?: string }> }
            | undefined;
          const nameProp = props?.propConcept?.find(
            (p) => p.propName === "RxNorm Name",
          );
        ...
        for (const result of results) {
          const interactionTexts =
            (result.drug_interactions as string[]) ?? [];
          const openfda = result.openfda as
            | { brand_name?: string[] }
            | undefined;
