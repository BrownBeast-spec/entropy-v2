...
export function registerOpenFdaTools(server) {
    // ─── 1. Check Drug Safety ────────────────────────────────────────────
    server.tool("check_drug_safety", "Check drug safety including label, boxed warning, contraindications and indications from OpenFDA.", {
        ...
        try {
            ...
            const getText = (v) => Array.isArray(v)
                ? v.join(" ").trim()
                : v
                    ? String(v).trim()
                    : "N/A";
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            dosage: getText(r["dosage_and_administration_table"] ??
    ...
    server.tool("check_adverse_events", "Get the top reported adverse reaction terms for a drug from OpenFDA FAERS.", {
    ...
    // ─── 3. Check Recalls ────────────────────────────────────────────────
    server.tool("check_recalls", "Get recent FDA enforcement actions (recalls) for a drug.", {
    ...
    // ─── 4. Get NDC Info ──────────────────────────────────────────────────
    server.tool("get_ndc_info", "Get National Drug Code (NDC) directory info — labeler, dosage form, active ingredients.", {
    ...
    // ─── 5. Search Drugs@FDA ──────────────────────────────────────────────
    server.tool("search_drugs_fda", "Search the Drugs@FDA database for approved drug products and their regulatory history.", {
    ...
    server.tool("get_drug_shortages", "Get current FDA drug shortage information, critical for supply chain context.", {
        drug: z
            .string()
            .optional()
        ...
        try {
            ...
            if (drug)
                params["search"] = `product_description:${drug}`;
...
//# sourceMappingURL=openfda.js.map