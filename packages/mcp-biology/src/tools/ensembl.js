...
export function registerEnsemblTools(server) {
    // ─── 1. Get Gene Info ───────────────────────────────────────────────
    server.tool("get_gene_info", "Lookup gene information by symbol in Ensembl (ID, biotype, coordinates).", {
        symbol: z.string().describe("Gene Symbol (e.g. 'BRCA1')"),
    }, async ({ symbol }) => {
        try {
            const data = (await ensemblFetch(`/lookup/symbol/homo_sapiens/${symbol}`));
            if (!data) {
                return {
                    content: [
                        {
    ...
    // ─── 2. Get Sequence ────────────────────────────────────────────────
    server.tool("get_sequence", "Retrieve genomic sequence by Ensembl ID (truncated to 500bp).", {
        geneId: z.string().describe("Ensembl Gene ID (e.g. 'ENSG00000139618')"),
    }, async ({ geneId }) => {
        try {
            const data = (await ensemblFetch(`/sequence/id/${geneId}`));
            if (!data) {
                return {
                    content: [
                        {
    ...
    // ─── 3. Get Variation ───────────────────────────────────────────────
    server.tool("get_variation", "Get variation information by variant ID (e.g., rs ID) including consequence and allele frequency.", {
        ...
        try {
            ...
            const data = (await ensemblFetch(`/variation/${sp}/${variantId}`));
            if (!data) {
                return {
                    content: [
                        {
    ...
    // ─── 4. Get Homology ────────────────────────────────────────────────
    server.tool("get_homology", "Get orthologs/paralogs (homology information) for a gene.", {
        geneId: z.string().describe("Ensembl gene ID (e.g. 'ENSG00000139618')"),
        species: z
            .string()
            .optional()
        ...
        try {
            ...
            const data = (await ensemblFetch(`/homology/id/${sp}/${geneId}`, params));
            if (!data?.data?.length) {
                return {
                    content: [
                        {
    ...
    // ─── 5. Get Xrefs ──────────────────────────────────────────────────
    server.tool("get_xrefs", "Get cross-references for a gene (links to other databases).", {
