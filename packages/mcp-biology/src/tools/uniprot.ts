...
interface UniProtEntry {
  ...
  comments?: Array<{
    ...
    reaction?: { name?: string; ecNumber?: string };
...
function getProteinName(entry: UniProtEntry): string | null {
  return (
...
function getGeneName(entry: UniProtEntry): string | null {
  return entry.genes?.[0]?.geneName?.value ?? null;
}

function getFunctionDescription(entry: UniProtEntry): string | null {
...
export function registerUniprotTools(server: McpServer): void {
  ...
  server.tool(
    "get_protein_data",
    "Fetch protein entry by UniProt accession (e.g., P00533 for EGFR). Returns accession, protein name, gene name, organism, sequence length, and function description.",
    {
      accession: z.string().describe("UniProt accession (e.g. 'P00533')"),
    },
    async ({ accession }) => {
      try {
        const data = (await uniprotFetch(
          `/${accession}.json`,
        )) as UniProtEntry | null;
  ...
  server.tool(
    ...
    "Fetch functional annotations for a protein from UniProt. Parses FUNCTION, CATALYTIC_ACTIVITY, and SUBCELLULAR_LOCATION comments.",
    {
      accession: z.string().describe("UniProt accession (e.g. 'P00533')"),
    },
    async ({ accession }) => {
      try {
        ...
        const subcellularLocations = comments
          ...
          .flatMap(
            (c) =>
              c.subcellularLocations?.map((sl) => sl.location?.value) ?? [],
          )
          .filter(Boolean);
  ...
  server.tool(
    "search_uniprot",
    "Search UniProt by query string. Returns top 5 results with accession, protein name, and organism.",
    {
