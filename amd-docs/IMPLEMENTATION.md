...
- Added brief first-query onboarding: quick input strip on `WorkspaceQueriesPage` (query text + mode + "Run first query"), in-query onboarding progress card in `WorkspaceQueryView`, and right-panel onboarding cues in `RightChatPanel` that reinforce `enter query -> fetch sources -> add evidence -> graph augmented` flow
- Reworked workspace flow into an editorial command-deck UX: `WorkspacesPage` now uses a bold workspace-card deck (replacing table layout), removed the "query mode is selected inside workspace" tag, and refreshed create-workspace panel; `WorkspaceQueriesPage` replaced mode-specific big buttons with a multiline chat-style query composer (Researcher/Strategist mode chips + single Run Query CTA) and requires explicit query input before navigating to query view; `WorkspaceQueryView` now focuses on rendering query results without extra onboarding banner
- Lab Notebook add-to-graph path is now backend-workspace aligned: `RightChatPanel` uses `addNodesToWorkspace(workspaceId, { nodes, queryId, inferEdges: true })` from `src/lib/api/workspace` instead of legacy `/api/workspace/add-nodes`, and query contribution metadata continues to update from returned node/edge IDs
- Manual search graph snapshot payload now includes `nodeTypes` and `existingConcepts`, and backend helpfulness scoring consumes these fields to avoid awarding novelty points for already-present concepts; added regression coverage in `apps/api/src/__tests__/search.test.ts` and `apps/mastra-app/src/__tests__/helpfulness-agent.test.ts`
...
- Intermediate report citations now render inline by replacing bracketed node-id anchors in section content (e.g. `[result_xxx]`) with mapped clickable citation controls that use section citation labels (with workspace-node fallback), and clicking inline citations now opens `EntityDetailDrawer` while preserving graph highlight behavior; added regression tests in `entropy-research-hub/src/components/workspace/IntermediateReportPanel.citations.test.tsx` and `entropy-research-hub/src/pages/WorkspaceView.inline-citation-drawer.test.tsx`
- Refined intermediate report UX: removed fixed-width report content constraint (`max-w-3xl`), suppressed duplicated top-line section heading text when content repeats the section title (e.g. `Overview`), and tightened inline citation visual ergonomics with truncation and spacing (`max-w-[220px]`, ellipsis, `mr-1 mb-1`) for multi-tag readability
- Changed report panel scroll behavior to default non-sticky flow: header controls + overview metric cards now scroll together with section body content (single `overflow-y-auto` container around panel content)
- Fixed notebook/report data-shape regressions: `WorkspaceContext` hydration now preserves existing local `query.report` when backend query metadata refreshes (preventing workflow synthesis output from disappearing in `IntermediateReportPanel`), and `RightChatPanel` now safely handles search entries missing `searchedSources` with fallback source counting to avoid runtime crashes; added regression tests in `WorkspaceContext.test.tsx`, `search.test.ts`, and `RightChatPanel.test.tsx`
- Fixed manual search entity-type normalization gap causing workspace add-node validation errors (`POST /api/workspace/:id/nodes` rejecting `type: "targets"`): frontend search adapter now maps entropy result type `targets` to graph-compatible `gene`, with regression coverage in `search.test.ts` and `RightChatPanel.test.tsx`
