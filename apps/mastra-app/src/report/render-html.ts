...
function parsePipeTable(lines: string[]): string {
  // Filter out separator lines (|---|---|) to detect header vs body rows
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isSep(line)) {
      hasHeader = true; // the row before this is the header
      continue;
    }
    // Split on | and trim, dropping the first/last empty cells from leading/trailing |
    const cells = line
      .split("|")
      .slice(1, -1)
      .map((c) => c.trim());
  ...
  const renderCell = (cell: string, tag: "th" | "td") => {
    // Apply inline markdown (bold/italic/code) inside each cell
    const inner = cell
...
function prose(text: string): string {
  ...
  // ── Pre-pass: extract pipe-table blocks ──────────────────────────────────
  // A pipe-table block is a run of lines where every line starts and ends with |
  ...
  return segments
    .map((seg) => {
      ...
      // Text segment — apply standard inline/block markdown
...
export function renderHtmlReport(input: ReportInput): string {
  ...
  const decisionClass = reviewerDecision.approved
    ? "decision-approved"
    : "decision-rejected";
  const decisionText = reviewerDecision.approved ? "✓ APPROVED" : "✗ REJECTED";
...
<p><strong>Notes:</strong> ${h(reviewerDecision.notes)}</p>
